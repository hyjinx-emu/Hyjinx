            ...
            if (!_isAddingCompleted)
            {
                ...
                if (_messagesDropped > 0)
                {
                    _messageQueue.Enqueue(new LogMessageEntry(
                        $"{_messagesDropped} message(s) dropped because of queue size limit. Increase the queue size or decrease logging verbosity to avoid this. You may change `ConsoleLoggerQueueFullMode` to stop dropping messages.",
                        logAsError: true
                    ));
